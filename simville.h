...
public:
	...
	**/
    ...
    ////////
    ///////
    ////////MOVE IS THE MAIN FUNCTION TO WORK ON!!!! 
	/** //move()
		...
		while (mov != pop->end()){
			...
			if (current->getTurnOver() == false){
				if (current->getStatus() == "Ignorant"){
					...
					case 0:
						moveTo = connect['n']; //move north is random is 0
						break;
					case 1:
						moveTo = connect['e']; //move east if random is 1
						break;
					case 2:
						moveTo = connect['s']; //move south if random is 2
						break;
					case 3:
						moveTo = connect['w']; //move west if random is 3
					}
                    std::list<District*>::iterator dist = districts.begin();
                    District* distr = *dist;
                    while (distr->getName != moveTo){
                        distr++;
                    }
